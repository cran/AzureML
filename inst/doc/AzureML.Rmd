...
- If the workspace location is not the default one (South Central US), we also pass the Azure ML web service management url. The url is of the form "https://[region].management.azureml.net", where the region is 'ussouthcentral' for South Central US, 'europewest' for West Europe, and 'asiasoutheast' for Southeast Asia.

Alternatively, to avoid passing the workspace ID, the authorization token and the url to each function, we can define the workspace settings in a file ~azureml/settings.ini as illustrated the example below.
<img src="settings.png" width="60%">

Next, call the function loadSettings()
```{r load, eval=FALSE}
loadSettings()
```
to load the workspace parameters into the R environment. Then the workspace ID, authorization token and url can be omitted in function calls. For example, the function publishWebService() can now be called as:
```{r publishAfterLoad, eval=FALSE}
response <- publishWebService("add", "addOnline", 
